...
class _ChatPageState extends State<ChatPage> {
  ...
  String searchUser = '';
  TextEditingController seatchTextEditingController =
  ...
  Future<void> searchLoadRepositories() async {
    String url = "http://localhost:3000/users?name_like=${searchUser}";
    http.Response response = await http.get(Uri.parse(url));
    if (response.statusCode == 200) {
      setState(() {
        dataRepositories = json.decode(response.body);
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Padding(
              ...
              child: TextField(
                controller: seatchTextEditingController,
                onChanged: (value){
                  if(value.length <= 0){
                    loadRepositories();
                  }else{
                    this.searchUser = seatchTextEditingController.text;
                    searchLoadRepositories();
                  }
                },
            ...
            ListView.builder(
              ...
              itemBuilder: (context, index){
                return ConversationList(
                  name: dataRepositories[index]['name'],
                  messageText: dataRepositories[index]['messageText'],
                  imageUrl: dataRepositories[index]['imageURL'],
                  time: dataRepositories[index]['time'],
                  isMessageRead: (index == 0 || index == 3)?true:false,
                  chatId: dataRepositories[index]['chatId'],
                );
              },
            ),
          ],
